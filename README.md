...
1. Push this repo to GitHub (`github.com/fromsukong/services`).
...
3. Push to `main` (or run the workflow manually). It runs `npm run build` and deploys `dist/` to the `fromsukong` Pages project (auto-created on first deploy, served at `fromsukong.pages.dev`).
4. Bind the domain: Cloudflare dashboard → **Workers & Pages** → `fromsukong` → **Custom domains** → add `fromsukong.com` and `www.fromsukong.com` (DNS records are created automatically).
